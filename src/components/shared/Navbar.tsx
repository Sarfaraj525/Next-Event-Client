...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 z-50 w-full shadow-sm backdrop-blur-md bg-slate-900/30">
        ...
        <ul className="hidden gap-8 text-sm font-medium md:flex">
          {navLinks.map((link) => (
            <li key={link.href}>
              <Link
                href={link.href}
                className="navButton !bg-white/10 hover:!bg-blue-500/20"
              >
                {link.label}
              </Link>
            </li>
              ...
              <>
                {user.role === "ADMIN" ? (
                  ...
                  <Link href="/profile/personal-info" className="relative">
                    <Avatar className="cursor-pointer ring-2 ring-blue-400 hover:ring-4 transition duration-300">
